...
class BreathingAnimation extends Component {
  ...
  render() {
    ...
    const breathStyle = {
      ...
      backgroundColor: breathAnim.interpolate({
        inputRange: [0, 1],
        outputRange: ['blue', 'white'], // Define a cor de preenchimento da bola maior
      }),
    };
    ...
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#8896d7' }}>
        ...
        >
          <Text style={{ color: 'white' }}>{isAnimating ? 'Pausar' : 'Começar'}</Text>
        </TouchableOpacity>
      </View>
    );
  }
